---
layout: player
profile_type: "video"

title: "Szymon Łyczko (2006) – Stal Rzeszów | Betclic 1 Liga, Poland (2nd Tier)"

youtube: "https://www.youtube.com/watch?v=UY_J1ntCGJ0"

report_by: "Scouty Pippen"
report_date: "08.01.2025"
site_added: "09.08.2026"

position: Right Winger

nationalities:
  - Poland
  - USA

birth_date: "2006-02-08"
height_cm: 178

dominant_foot: "Right"
two_footed_ability: "Strong"

positions:
  - role: "Right Winger"
    side: "right"
    rank: 1
  - role: "Left Winger"
    side: "left"
    rank: 2

summary: |
  🇵🇱 Szymon Łyczko | 18 y/o RW/LW
  ⚡ Fast, agile winger with explosive acceleration & strong 1v1 ability
  🪄 Sharp dribbler with the creativity to threaten from either flank
  🦶 Precise right foot and a strong left for crosses, passes, shots & finishes
  🎯 Dangerous delivery with flashes of vision beyond his age
  📈 Poland youth international with the raw tools to succeed at a higher level

report: |
  This player profile is based on video scouting by Scouty Pippen. A full written scouting report is not currently available.

  Szymon Łyczko is a fast, agile winger who thrives in 1v1 situations. He combines explosive acceleration with sharp dribbling skills and a creative mindset, making him a constant threat down either flank.

  He is comfortable on both wings, delivers dangerous crosses and shows flashes of vision beyond his age. His right foot is precise, while he also uses his left effectively for crosses, passes, shots and finishes.

  Łyczko is a quick winger with very good dribbling ability and a precise right foot. He has also represented Poland at U17 and U18 level.

  While still developing, he already possesses the raw tools needed to succeed at a higher level. Greater consistency in his decision-making and goal contributions would help him take the next step.

  Playing in Poland’s second tier, Łyczko looks ready for a bigger challenge. Clubs looking for an exciting, modern winger with upside should be paying close attention.

transfermarkt: "https://www.transfermarkt.com/szymon-lyczko/profil/spieler/1108661"
sofascore: "https://www.sofascore.com/football/player/szymon-lyczko/1435357#tab:season"

podcast: "https://open.spotify.com/episode/5CmgszNPCudqf8ap0EXXkO"
podcast_timestamp: "29:07"
---
