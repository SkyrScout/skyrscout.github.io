...
const target = safeStaffTarget(params.get("next") || "/foyer/");
...
async function initializeEntrance() {
  ...
  if (session.status === "authorized") return showAuthorized(session.profile);
  if (session.status === "not-configured") {
    setState("error", "Staff Entrance is not connected to Firebase yet.");
    showError("The Firebase web configuration must be connected before staff can sign in.");
    return;
  }
  if (session.status === "denied") { await signOutStaff(); return showDenied(); }
...
signInButton.addEventListener("click", async () => {
  clearError(); signInButton.disabled = true; setState("checking", "Waiting for Google sign-in…");
  try {
    const session = await signInApprovedGoogleAccount();
    if (session.status === "authorized") { showAuthorized(session.profile); window.location.assign(target); return; }
    showDenied();
  } catch (error) {
...
enterButton.addEventListener("click", () => window.location.assign(target));
signOutButton.addEventListener("click", async () => { await signOutStaff(); showSignedOut(); });
initializeEntrance();
